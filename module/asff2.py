...
class ASFF_3(nn.Module):
    def __init__(self, inter_dim=512, level=0, channel=[64, 128, 256,512]):
        ...
        # self.dsconv = DepthwiseSeparableConv(in_channels=self.inter_dim, out_channels=1024)

        self.level = level
        if self.level == 2:
            # self.downsample2x = Downsample(channel[0], channel[1], scale_factor=2)
            self.downsample4x = Downsample(channel[0], channel[1], scale_factor=4)
            # self.downsample8x = Downsample(channel[0], channel[1], scale_factor=8)
            ######rga
    ...
    def forward(self, x1,x2):
        ...
        if self.level == 0:
            ...
            # input1 = self.downsample2x(input1)
